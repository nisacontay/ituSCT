...
public:
     ...
     */
    Matrix<T> operator-(const Matrix<T>& vector2){
        bool sameSize = sameDimension(vector2); // check if the dimensions are the same
        if(!sameSize){
            throw std::invalid_argument("Dimensions are not matched!");
        } 
    ...
    Matrix<double> inverse(){
        if(row != column && row > 3){ // check if the matrix is squared and less than 3
            throw std::invalid_argument("The matrix should be squared and less than 3!");
        }
        else if(determinant() == 0){ // check if the matrix is non-singular
            throw std::invalid_argument("The matrix should be non-singular!");
        }else{
            T det = determinant();
    ...
    T trace(){ // trace of a matrix
        if(row != column){ // check if the matrix is squared
            throw std::invalid_argument("The matrix should be squared!");
        }  else{
            T total = 0;
            for(int i = 0; i < row; i++){
    ...
    T determinant(){ // determinant of a matrix
        if(row != column && row > 3){ // check if the matrix is squared and less than 3
            ...
            if(row == 1){ // determinant of a 1x1 matrix
                ...
                return det;
            } else { // determinant of a 3x3 matrix
     ...
     */
    bool sameDimension(const Matrix<T>& vector2){ // check if the dimensions are the same
     ...
     */
    Matrix<T> multiply(const Matrix<T>& vector2){ // multiply two matrices
     ...
     */
    Matrix<T> dot(const Matrix& vector2){ // dot product of two matrices
    ...
    static Matrix<T> identity(int num){ // create an identity matrix
        if(num < 1) throw std::invalid_argument("Number should be positive!"); 
        else {
